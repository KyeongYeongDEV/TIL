...
> HTTP/1.1은 기본적으로 동기식 요청/응답 구조로, 한 연결에서 한 번에 하나의 요청만 처리할 수 있습니다.
이를 보완하기 위해 Keep-Alive, 파이프라이닝 같은 기술이 있었지만 **성능 제약이 많았습니다.**
반면 HTTP/2 는 **단일 TCP 연결에서 다중 요청을 동시에 처리할 수 있는 Multiplexing(다중화)** 기능이 도입되어 
**지연시간과 대역폭 활용이 크게 개선** 되었습니다.
또한 **헤더 합축(HPACK), 서버 푸시** 기능도 추가돼 웹 성능 최적화에 효과적입니다.
> 

| 항목 | HTTP/1.1 | HTTP/2 |
| --- | --- | --- |
| 요청 처리 | 순차 처리 (1 요청 = 1 응답) | 멀티플렉싱 (동시 다중 처리 가능) |
| 연결 수 | 여러 TCP 연결 필요 | 하나의 TCP 연결로 모두 처리 가능 |
| 헤더 | 반복 전송, 비효율적 | HPACK 압축으로 효율화 |
| 서버 Push | 없음 | 가능 (브라우저에 자원 미리 보내줌) |

## 1. HOL Blocking 에 대해 설명해 주세요.

> HOL(Head Of Line) Blocking은 **처리 지연 현상**입니다.
먼저 도착한 요청이나 패킷이 막혀 있으면, **뒤에 대기 중인 요청들까지 모두 대기하게 되는 상황**을 말합니다.
HTTP/1.1에서는 한 TCP 연결에 요청을 파이프라이닝해도 **응답 순서를 보장해야 하므로**, 
앞 요청이 늦어지면 뒤 요청도 막히게 됩니다.

HTTP/2는 멀티플렉싱으로 어느 정도 HOL 문제를 해결했지만, **여전히 TCP 레벨에서의 HOL Blocking은 존재**합니다.
이 문제를 **UDP 기반의 HTTP/3에서 완전히 제거하려고 했습니다.**
> 

## 2. HTTP/3.0의 주요 특징에 대해 설명해 주세요.

> HTTP/3dms **TCP**가 아닌 **UDP** 기반의 **QUIC 프로토콜**을 사용한다는 게 가장 큰 차이점입니다.
QUIC는 TCP + TLS + HTTP2의 기능을 통합하면서도 **지연시간 감소와 연결 복원성을 대폭 향상**시킵니다.
특히, **패킷 손실이 생겨도 전체 연결에 영향 없이 스트림별로 독립적으로 복구**되기 때문에 HOL **Blocking 문제가 완전히 해결**됩니다.

그리고 핸드쉐이크가 0-RTT로 줄어들면서, 기존보다 **더 빠른 연결 설정**이 가능합니다.
> 

| 항목 | HTTP/3 |
| --- | --- |
| 기반 프로토콜 | TCP → **UDP 기반 QUIC** |
| HOL Blocking | **스트림 단위 분리로 완전 제거** |
| 핸드쉐이크 | 0-RTT 지원 (즉시 통신 가능) |
| 연결 복원성 | 중간 네트워크 변경 시에도 재연결 불필요 |
| TLS 적용 | QUIC 자체에 TLS 내장 (기본 암호화) |

✅ HTTP/1.1 → 순차 처리

✅ HTTP/2 → 멀티플렉싱 + TCP 기반

✅ HTTP/3 → QUIC 기반으로 HOL 제거, 더 빠르고 안정적

### 실제로 HTTP/3는 어느 브라우저에서 쓰이나요?

> 네, 현재 대부분의 모던 웹 브라우저에서는 HTTP/3를 지원하고 있습니다.
> 
> 
> 예를 들어, **Chrome, Edge, Firefox, Safari 모두 HTTP/3를 기본적으로 지원**하고 있고,
> 
> 실제로 Google, Facebook, Cloudflare 같은 대형 서비스들은 이미 HTTP/3 기반으로 트래픽을 처리하고 있습니다.
> 
> 다만, HTTP/3는 **UDP 기반의 QUIC을 사용하기 때문에**,
> 
> 서버 측에서도 이를 지원하는 웹 서버(NGINX, Cloudflare, Caddy 등)나 리버스 프록시 설정이 필요합니다.
> 
> 정리하자면, 브라우저는 대부분 준비돼 있고, **서버 환경에서도 적용이 점차 확산 중**입니다.
>
